...
MushroomObserver::Application.configure do
  ...
  # and use secure cookies. Off -- SSL/TLS termination is an open
  # decision for the Kamal front routing tier, see #5345.
  config.force_ssl = false

  # Under Docker/Kamal (#5345, same ENV["PORT"] check as config/puma.rb)
  # a container's filesystem doesn't survive a redeploy, and
  # docker logs/Kamal only capture STDOUT -- log there instead of a
  # file that would be silently lost every deploy. New 7.1 logging
  # uses BroadcastLogger; not using TaggedLogging yet.
  output = ENV["PORT"] ? $stdout : "log/production.log"
