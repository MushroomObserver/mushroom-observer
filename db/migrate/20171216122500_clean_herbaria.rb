...
# Frozen 2017 data-cleanup migration, already run in production; only
# ever replays on a from-scratch database rebuild. Pre-existing
# complexity/line-length predates the User.current removal touched
# here - not refactoring a decade-old one-time script for style.
...
class CleanHerbaria < ActiveRecord::Migration
  ...
  def up
    ...
    Herbarium.where.not(code: VALID_CODES).find_each do |herbarium|
      Rails.logger.debug do
        "Removing code #{herbarium.code.inspect} from #{herbarium.name.inspect}"
      end
      herbarium.update_columns(code: "")
    end

    # Somehow HTML formatting ended up getting stored in there for a few.
    Herbarium.where("name LIKE '%<%'").find_each do |herbarium|
      new_name = herbarium.name.gsub(/<[^<>]*>/, "")
      Rails.logger.debug { "Removing HTML tags from #{herbarium.name.inspect}" }
      herbarium.update_columns(name: new_name)
    end

    # Some names have leading and trailing space, sigh.
    Herbarium.where("name LIKE ' %' OR name LIKE '%  %' OR name LIKE '% '").find_each do |herbarium|
      new_name = herbarium.name.gsub(/\s\s+/, " ").gsub(/^\s+|\s+$/, "")
      Rails.logger.debug do
        "Removing excess white space from #{herbarium.name.inspect}"
      end
      herbarium.update_columns(name: new_name)
    end
    ...
    Herbarium.connection.select_rows(%(
      ...
      Rails.logger.debug do
        "Merging #{count} herbaria with the name #{name.inspect}"
      end
      last_herbarium = nil
      Herbarium.where(name: name).find_each do |herbarium|
        if last_herbarium
          Rails.logger.debug do
            "Merging #{herbarium.id} with #{last_herbarium.id}"
          end
    ...
    Rails.logger.debug("Making users owner of their own herbarium.")
    Herbarium.connection.select_rows(%(
      ...
      if old_user_id.present? && old_user_id != user.id
        ...
        Rails.logger.debug do
          "Making #{user.login.inspect} owner of #{name.inspect} (was #{old_user.inspect})"
        end
      end
    ...
    Herbarium.connection.select_rows(%(
      ...
      Rails.logger.debug { "#{user.login.inspect} has #{count} herbaria!" }
      last_herbarium = nil
      Herbarium.where(personal_user_id: user_id).find_each do |herbarium|
        if last_herbarium
          Rails.logger.debug do
            "Merging #{herbarium.name.inspect} with #{last_herbarium.name.inspect}"
          end
          herbarium = herbarium.merge(last_herbarium)
        end
        last_herbarium = herbarium
    ...
    Rails.logger.debug("Adding users as curator of their personal herbarium")
    ...
    Herbarium.connection.execute(%(
      ...
      #{personal_herbaria.map do |h|
        "(#{h.id},#{h.personal_user_id})"
      end.join(",")}
    ))
    ...
    Rails.logger.debug("Removing all email addresses")
    Herbarium.connection.execute(%(
    ...
    [12, 44, 101, 103, 108, 137, 145, 149, 203, 210, 212, 213, 214, 299, 328, 343, 363, 371, 372,
     ...
     822, 823, 827, 830, 831, 851, 853, 854, 856, 858, 860, 862, 867, 884, 899, 906].each do |id|
      ...
      Rails.logger.debug { "Destroying herbarium #{comment.inspect}" }
      garbage_herbarium.destroy
      HerbariumRecord.where(herbarium_id: id).find_each do |herbarium_record|
        if herbarium_record.observations.none?
          Rails.logger.debug do
            "Destroying unused herbarium record #{herbarium_record.id}"
          end
        ...
        new_notes = "[This record used to be at an herbarium called " \
                    ...
                    gsub(/  +/, " ").gsub(/\a\s+|\s+\z/, "")
        Rails.logger.debug do
          "Adding comment #{comment.inspect} to herbarium record ##{herbarium_record.id}"
        end
        herbarium_record.update(
