...
class AccountControllerTest < FunctionalTestCase
  ...
  def test_edit_prefs
    ...
    assert_equal(:above_species, user.hide_authors.to_sym)
    assert_equal(:keep_but_hide, user.keep_filenames.to_sym)
    assert_equal(100, user.layout_count)
    assert_equal(licenses(:publicdomain), user.license)
    assert_equal("el", user.locale)
    assert_equal(:scientific, user.location_format.to_sym)
