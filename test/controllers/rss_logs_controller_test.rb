...
class RssLogsControllerTest < FunctionalTestCase
  ...
  def test_altering_types_shown_by_rss_log_index
    ...
    # Check new type links
    # Be sure "all" loads some rss_logs!
    get(:index, params: { q: { type: "all" } })

    get(:index, params: { q: { type: [:article, :glossary_term] } })
    assert_template(:index)

    get(:index, params: { q: { type: [] } })
    assert_template(:index)

    # Check old links with top level :type param.
    # Not generated by MO anymore. Should not exist unless bookmarked
