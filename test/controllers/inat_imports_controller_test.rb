...
class InatImportsControllerTest < FunctionalTestCase
  ...
  def test_new_inat_import
    ...
    assert_select(
      "textarea#inat_import_inat_ids", true,
      "Form needs a textarea for inputting iNat ids"
    )
    assert_select(
      "input#inat_import_inat_username", true,
  ...
  def test_new_inat_import_already_importing
    ...
    assert_flash_warning(
      "Should flash warning if user starts iNat import " \
      "while another is running"
    )
    assert_redirected_to(
      inat_import_path(import,
  ...
  def test_create_cancel_reset
    ...
    assert(import.canceled?,
           "Test needs a canceled InatImport fixture")
    id = "123"
    params = inat_import_params(
      inat_ids: id,
  ...
  def test_create_missing_username
    user = users(:rolf)
    id = "123"
    params = inat_import_params(inat_ids: id)
  ...
  def test_create_no_observations_designated
    params = inat_import_params(
      inat_username: "anything", inat_ids: "",
      consent: 1
    )
    login
    assert_no_difference(
      "Observation.count",
      "Imported observation(s) though none designated"
    ) do
      post(:create, params: params)
    end
  ...
  def test_create_list_and_all
    params = inat_import_params(
      inat_username: "anything", inat_ids: "7,8,9",
      import_all: 1, consent: 1
    )
    login
  ...
  def test_create_illegal_observation_id
    params = inat_import_params(
      inat_username: "anything", inat_ids: "123*",
      consent: 1
    )
    login
    assert_no_difference(
  ...
  def test_create_no_consent
    ...
    assert_no_difference(
      "Observation.count",
      "iNat obss imported without consent"
    ) do
      post(:create, params: params)
    end
  ...
  def test_allows_maximum_ids
    ...
    assert_equal(
      0, inat_import.total_imported_count.to_i,
      "Test needs InatImport fixture without prior imports"
    )

    id = 1_234_567_890
    reps = InatImportsController::Validators::MAX_ID_LIST_SIZE /
           id.to_s.length
  ...
  def test_create_too_many_ids_listed
    ...
    id_list = ""
    id = 1_234_567_890
    max = InatImportsController::Validators::MAX_ID_LIST_SIZE
    id_list += "#{id += 1}," until id_list.length > max

    params = inat_import_params(
      inat_username: "anything", inat_ids: id_list,
      consent: 1
    )
  ...
  def test_create_previously_imported
    ...
    params = inat_import_params(
      inat_username: "anything", inat_ids: inat_id,
      consent: 1
    )
    login
    assert_no_difference(
      "Observation.count",
      "Imported a previously imported iNat obs"
    ) do
      post(:create, params: params)
    end

    assert_flash_text(/#{:inat_previous_import.l(count: 1)}/)
    # It should continue even if some ids were previously
    # imported. The job will exclude previous imports via the
    # iNat API `without_field: "Mushroom Observer URL"` param.
  ...
  def test_create_strip_inat_username
    user = users(:mary)
    assert(
      APIKey.where(user: user, notes: MO_API_KEY_NOTES).none?,
      "Test needs user fixture without an MO API key " \
      "for iNat imports"
    )
  ...
  def test_create_authorization_request
    ...
    assert_redirected_to(INAT_AUTHORIZATION_URL)
    assert_equal(
      inat_ids.split(",").length,
      inat_import.reload.importables,
      "Failed to save InatImport.importables"
    )
    assert_equal(
      "Authorizing", inat_import.reload.state,
      "MO should be awaiting authorization from iNat"
    )
    assert_equal(
      InatImport.sum(:total_seconds) /
        InatImport.sum(:total_imported_count),
  ...
  def test_allow_first_time_import_all
    ...
    assert_nil(user.inat_username,
               "Test needs fixture without inat_username")
    params = inat_import_params(
  ...
  def test_import_all_anothers_observations
    ...
    params = inat_import_params(
      inat_username: "anything", inat_ids: nil,
      consent: 1, import_all: 1
    )
  ...
  def test_super_importer_can_import_specific_ids_from_another_user
    ...
    params = inat_import_params(
      inat_username: "other_inat_user",
      inat_ids: "12345", consent: 1
    )
  ...
  def test_import_authorized
    ...
    assert_blank(user.inat_username,
                 "Test needs user fixture without " \
                 "an iNat username")
    inat_import = inat_imports(:rolf_inat_import)

    # empty id_list to prevent importing any observations
    ...
    assert_in_delta(
      ...
      "When job starts, elapsed time for 1st import " \
  ...
  def test_inat_username_unchanged_if_authorization_denied
    ...
    assert_blank(
      ...
      "User inat_username shouldn't change if user " \
      "denies authorization to MO"
    )
  end
  ...
  def test_cancel
    ...
    assert(import.job_pending? && !import.canceled?,
           "Test needs a Import fixture with a " \
  ...
  # Wrap params under :inat_import key for Superform
  def inat_import_params(**attrs)
    { inat_import: attrs }
  end
