...
class AutocompletersControllerTest < FunctionalTestCase
  ...
  def test_auto_complete_location
    ...
    expect = m.sort.uniq
    expect.unshift("M")
    good_autocompleter_request(:new, type: :location, id: "Modesto")
    ...
    good_autocompleter_request(:new, type: :location, id: "Xystus")
  ...
  def test_auto_complete_herbarium
    ...
    expect = m.sort.uniq
    expect.unshift("D")
    good_autocompleter_request(:new, type: :herbarium, id: "Dick")
    assert_equal(expect, JSON.parse(@response.body))
  end

  def test_auto_complete_empty
    login("rolf")
    good_autocompleter_request(:new, type: :name, id: "")
  ...
  def test_auto_complete_name_above_genus
    login("rolf")
    expect = %w[F Fungi]
    good_autocompleter_request(:new, type: :clade, id: "Fung")
    assert_equal(expect, JSON.parse(@response.body))
  end
  ...
  def test_auto_complete_name
    ...
    good_autocompleter_request(:new, type: :name, id: "Agaricus")
    assert_equal(expect, JSON.parse(@response.body))

    good_autocompleter_request(:new, type: :name, id: "Umbilicaria")
  ...
  def test_auto_complete_species_list
    ...
    good_autocompleter_request(:new, type: :species_list, id: "List")
    assert_equal(["L", list1, list2, list3, list4], JSON.parse(@response.body))

    good_autocompleter_request(:new, type: :species_list, id: "Mojo")
    assert_equal(["M", list3], JSON.parse(@response.body))

    good_autocompleter_request(:new, type: :species_list, id: "Xystus")
    assert_equal(["X"], JSON.parse(@response.body))
  end

  def test_auto_complete_user
    login("rolf")
    good_autocompleter_request(:new, type: :user, id: "Rover")
    assert_equal(
    ...
    good_autocompleter_request(:new, type: :user, id: "Dodo")
    assert_equal(["D", "dick <Tricky Dick>"], JSON.parse(@response.body))

    good_autocompleter_request(:new, type: :user, id: "Komodo")
    assert_equal(["K", "#{katrina.login} <#{katrina.name}>"],
                 JSON.parse(@response.body))

    good_autocompleter_request(:new, type: :user, id: "Xystus")
    assert_equal(["X"], JSON.parse(@response.body))
  end

  def test_auto_complete_bogus
    login("rolf")
    bad_autocompleter_request(:new, type: :bogus, id: "bogus")
  end
end
