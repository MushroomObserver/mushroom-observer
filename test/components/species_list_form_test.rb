...
# Tests for Components::SpeciesListForm — the Phlex Superform that
# replaces app/views/controllers/species_lists/_form.html.erb plus the
# species_lists/form/_fields_for_project.erb sub-partial. Covers each
# field and the conditional rendering branches (clone_id present/absent,
# projects empty/populated, owner vs non-owner disable rules).
class SpeciesListFormTest < ComponentTestCase
  def setup
    super
    @user = users(:rolf)
  end
  ...
  def test_edit_flow_form_action_and_patch_method
    ...
    # Persisted SpeciesList → PATCH /species_lists/:id (via _method hidden).
    assert_html(html,
                "form#species_list_form[action='/species_lists/#{spl.id}']")
    assert_html(html,
  ...
  def test_clone_id_renders_when_set
    ...
    # `clone_id` is top-level, NOT under the species_list namespace —
    # the controller reads `params[:clone_id]`.
    assert_html(html,
  ...
  def test_approved_where_hidden_field_namespaced_under_species_list
    # `approved_where` is the dubious-confirmation flag. Pre-Phlex it
    # lived as a top-level URL query param on the form action; post-
    # Phlex it's a hidden field under the species_list namespace.
    spl = species_lists(:first_species_list)
    html = render_form(species_list: spl)
  ...
  def test_renders_submit_buttons_top_and_bottom
    ...
    # `submit(button.l, center: true)` is called twice — one above the
    # fields and one below. Locks in the count so a future refactor
    # doesn't silently drop one.
    assert_html(html, "input[type='submit'][value='Create']", count: 2)
  end
  ...
  def test_project_checkbox_disabled_for_non_owner_non_member
    ...
    # eol_project members: [rolf, mary, katrina] — dick is NOT a member.
    # bolete_project members: [mary, dick] — dick IS a member.
    # So dick should NOT be able to toggle eol_project (disabled) but
    # CAN toggle bolete_project (enabled).
  ...
  def test_owner_can_modify_all_project_checkboxes
    # When the @user IS the species_list owner, all checkboxes should
    # be enabled regardless of project membership.
    rolf_list = species_lists(:first_species_list)
  ...
  def test_renders_form_location_feedback_when_dubious_reasons_present
    # The FormLocationFeedback component shows the dubious-place
    # warning + an "approve anyway" button. The form just delegates;
    # it should pass `dubious_where_reasons` through.
    html = render_form(
      species_list: SpeciesList.new,
