...
class ModalLinkTest < ComponentTestCase
  def test_plain_modal_link_renders_link_with_stimulus_data_attrs
    html = render(Components::Link::Modal.new(
                    "comment", "Edit", "/edit"
                  ))
  ...
  def test_icon_modal_link_renders_through_icon_link
    ...
    # With `:icon`, delegates to Components::Link::Icon — the anchor
  ...
  def test_tab_kwarg_derives_name_path_opts
    ...
    html = render(Components::Link::Modal.new("project_summary", tab: tab))
