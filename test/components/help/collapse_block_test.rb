...
class CollapseHelpBlockTest < ComponentTestCase
  ...
  def test_renders_without_arrow_when_direction_nil
    html = render_component(
      Components::Help::CollapseBlock.new(target_id: "help_2")
    ) { "Content" }
  ...
  def test_renders_with_arrow_down_when_direction_down
    html = render_component(
      Components::Help::CollapseBlock.new(target_id: "help_3",
                                          direction: "down")
    ) { "Content" }
  ...
  def test_renders_arrow_visible_on_mobile_when_mobile_true
    html = render_component(
      Components::Help::CollapseBlock.new(
        target_id: "help_5",
        direction: "up",
        mobile: true
      )
  ...
  def test_yields_block_content
    html = render_component(
      Components::Help::CollapseBlock.new(target_id: "help_7")
    ) do
