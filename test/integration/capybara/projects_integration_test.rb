...
class ProjectsIntegrationTest < CapybaraIntegrationTestCase
  def test_add_project_dates
    ...
    # Add project, no start or end date
    title = "Project without start or end"
    visit(projects_path)
    click_on(:list_projects_add_project.l)
    fill_in(:form_projects_title.l, with: title)
    choose("start_date_fixed_false")
    assert_selector("#start_date_fixed_false[checked='checked']")
    choose("end_date_fixed_false")
    assert_selector("#end_date_fixed_false[checked='checked']")

    assert_difference("Project.count", 1, "Failed to create Project ") do
      click_on("Create")
    end
    project = Project.order(created_at: :asc).last
