...
class RedirectsTest < IntegrationTestCase
  def test_controller
    get("/article")
    assert_equal(articles_path, @response.request.fullpath)
  end
  ...
  def test_show_article
    ...
    assert_equal(article_path(Article.first.id), @response.request.fullpath)
  end

  def test_create_article_get
    login("article_writer", "testpassword", true)
    get("/article/create_article")
    assert_equal(new_article_path, @response.request.fullpath)
  end
  ...
  def test_destroy_article_patch
    ...
    assert_equal(article_url(Article.first.id), @response.header["Location"])
  end
