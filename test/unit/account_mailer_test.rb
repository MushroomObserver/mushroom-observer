...
################################################################################
  ...
  def test_email_1
    email = AccountMailer.create_admin_request(@katrina, @rolf,
      projects(:eol_project), 'Please do something or other', 'and this is why...').encoded
    fix_mac_vs_pc!(email)
  ...
  def test_email_2
    email = AccountMailer.create_author_request(@katrina, @rolf,
      names(:coprinus_comatus), 'Please do something or other', 'and this is why...').encoded
    fix_mac_vs_pc!(email)
  ...
  def test_email_3
    email = AccountMailer.create_comment(@dick, @rolf, observations(:minimal_unknown), comments(:another_comment)).encoded
    fix_mac_vs_pc!(email)
  ...
  def test_email_4
    email = AccountMailer.create_comment(@rolf, @mary,
      observations(:minimal_unknown), comments(:minimal_comment)).encoded
    fix_mac_vs_pc!(email)
  ...
  def test_email_5
    email = AccountMailer.create_commercial_inquiry(@mary, images(:commercial_inquiry_image),
    ...
    images(:commercial_inquiry_image).user.email_html = false
    email = AccountMailer.create_commercial_inquiry(@mary, images(:commercial_inquiry_image),
      'Did test_commercial_inquiry work?').encoded
  ...
  def test_email_6
    ...
    names(:coprinus_comatus).search_name = names(:coprinus_comatus).search_name.to_ascii
    email = AccountMailer.create_consensus_change(@dick, @mary, observations(:coprinus_comatus_obs), names(:agaricus_campestris), names(:coprinus_comatus), observations(:coprinus_comatus_obs).created).encoded
    fix_mac_vs_pc!(email)
  ...
  def test_email_9
    email = AccountMailer.create_name_change(@dick, @mary, names(:peltigera).modified, names(:peltigera), 1, 2, names(:peltigera).review_status).encoded
    fix_mac_vs_pc!(email)
  ...
  def test_email_9b
    ...
    email = AccountMailer.create_name_change(@dick, @mary, names(:peltigera).modified, names(:peltigera), 0, 1, names(:peltigera).review_status).encoded
    fix_mac_vs_pc!(email)
    assert_string_equal_file(email, "#{FIXTURES_PATH}/name_change2.html")
  end

  def test_email_10
    email = AccountMailer.create_name_proposal(@mary, @rolf, namings(:coprinus_comatus_other_naming), observations(:coprinus_comatus_obs)).encoded
    fix_mac_vs_pc!(email)
  ...
  def test_email_11
    email = AccountMailer.create_naming_for_observer(@rolf, namings(:agaricus_campestris_naming),
      notifications(:agaricus_campestris_notification_with_note)).encoded
    fix_mac_vs_pc!(email)
  ...
  def test_email_12
    email = AccountMailer.create_naming_for_tracker(@mary, namings(:agaricus_campestris_naming)).encoded
    fix_mac_vs_pc!(email)
  ...
  def test_email_14
    # The umlaut in Mull. is making it do weird encoding on the subject line.
    observations(:coprinus_comatus_obs).name.search_name = names(:coprinus_comatus).search_name.to_ascii
    observations(:coprinus_comatus_obs).name.display_name = names(:coprinus_comatus).display_name.to_ascii
    email = AccountMailer.create_observation_change(@dick, @mary, observations(:coprinus_comatus_obs),
      'date,location,specimen,is_collection_location,notes,thumb_image_id,added_image,removed_image',
      observations(:coprinus_comatus_obs).created).encoded
    fix_mac_vs_pc!(email)
    ...
    email = AccountMailer.create_observation_change(@dick, @mary, nil,
      '**__Coprinus comatus__** L. (123)', observations(:coprinus_comatus_obs).created).encoded
    fix_mac_vs_pc!(email)
  ...
  def test_email_15
    email = AccountMailer.create_observation_question(@rolf, observations(:detailed_unknown),
    ...
    observations(:detailed_unknown).user.email_html = false
    email = AccountMailer.create_observation_question(@rolf, observations(:detailed_unknown),
      'Where did you find it?').encoded
  ...
  def test_email_16
    email = AccountMailer.create_publish_name(@mary, @rolf, names(:agaricus_campestris)).encoded
    fix_mac_vs_pc!(email)
  ...
  def test_email_20
    email = AccountMailer.create_location_change(@dick, @mary, locations(:albion).modified, locations(:albion), 0, 1).encoded
    fix_mac_vs_pc!(email)
