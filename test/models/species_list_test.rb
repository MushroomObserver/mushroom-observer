...
class SpeciesListTest < UnitTestCase
  ...
  def test_construct_observation
    ...
    assert_operator(o.created_at, :>=, 1.minute.ago)
    assert_operator(o.updated_at, :>=, 1.minute.ago)
    ...
    assert_operator(n.created_at, :<=, now + 1.second)
    assert_operator(n.updated_at, :<=, now + 1.second)
    assert_users_equal(lone_wolf, n.user)
    assert_names_equal(name, n.name)
    assert_obj_arrays_equal([v], n.votes)
    assert_operator(v.created_at, :<=, now + 1.second)
    assert_operator(v.updated_at, :<=, now + 1.second)
