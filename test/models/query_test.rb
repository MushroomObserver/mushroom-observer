...
#    assert_equal([1, 2], Query.lookup(:User, :in_set, ids: %w(1 2)).params[:ids])
  ...
  # obs 2: imgs 1, 2
    ...
    # So inner1.outer should be all Observations with images, sorted by id
    results = inner1.outer.result_ids
    obs_with_images_ids = [observations(:detailed_unknown_obs).id,
                           observations(:coprinus_comatus_obs).id,
                           observations(:agaricus_campestris_obs).id,
                           observations(:peltigera_obs).id,
                           observations(:peltigera_rolf_obs).id,
                           observations(:fungi_obs).id,
                           ].sort
    assert_equal(obs_with_images_ids, results,
                 "inner1.outer results missing observations with images \
                  #{obs_with_images_ids - results}")

    inners_outer_obs_ids = [observations(:detailed_unknown_obs).id,
                            observations(:coprinus_comatus_obs).id,
                            observations(:agaricus_campestris_obs).id,
                            observations(:peltigera_obs).id].sort

    # old assertion: assert_starts_with([2, 3, 4, 12], results)
    # Following tests if results contain all inners_outer_obs_ids -- in order --
    # because both are sorted.
    expect = inners_outer_obs_ids - results
    assert_empty(expect,
                 "inner1.outer results missing observations #{expect}")

byebug
    obs_with_images.current_id = results[1]
