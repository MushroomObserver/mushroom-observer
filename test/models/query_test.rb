...
class QueryTest < UnitTestCase
  ...
  def test_results
    ...
    assert_equal(roy.location_format.to_sym, :scientific)
    assert_equal(
      Set.new,
  ...
  def test_location_ordering
    ...
    User.current = rolf
    assert_equal(:postal, User.current_location_format.to_sym)
    assert_query([albion, elgin_co], :Location, :in_set,
                 ids: [albion.id, elgin_co.id], by: :name)

    User.current = roy
    assert_equal(:scientific, User.current_location_format.to_sym)
