...
class PatternSearchTest < UnitTestCase
  ...
  def test_parse_string
    x = PatternSearch::Term.new(:xxx)
    x.vals = [];        assert_raises(PatternSearch::MissingValueError) { x.parse_string }
    x.vals = [1, 2];    assert_raises(PatternSearch::TooManyValuesError) { x.parse_string }
    x.vals = ["blah"];  assert_equal("blah", x.parse_string)
  end
  ...
  def test_parse_boolean
    ...
    x.vals = ["no"];    assert_raises(PatternSearch::BadYesError) { x.parse_boolean(:only_yes) }
  end

  def test_parse_float
    x = PatternSearch::Term.new(:xxx)
    x.vals = [];         assert_raises(PatternSearch::MissingValueError) { x.parse_float(-10, 10) }
    x.vals = [1, 2];     assert_raises(PatternSearch::TooManyValuesError) { x.parse_float(-10, 10) }
    x.vals = ["xxx"];    assert_raises(PatternSearch::BadFloatError) { x.parse_float(-10, 10) }
    x.vals = ["-10.1"];  assert_raises(PatternSearch::BadFloatError) { x.parse_float(-10, 10) }
    x.vals = ["10.1"];   assert_raises(PatternSearch::BadFloatError) { x.parse_float(-10, 10) }
    x.vals = ["-10"];    assert_equal(-10, x.parse_float(-10, 10))
    x.vals = ["10"];     assert_equal(10, x.parse_float(-10, 10))
    x.vals = [".123"];   assert_equal(0.123, x.parse_float(-10, 10))
    x.vals = ["-.123"];  assert_equal(-0.123, x.parse_float(-10, 10))
    x.vals = ["1.234"];  assert_equal(1.234, x.parse_float(-10, 10))
    x.vals = ["-1.234"]; assert_equal(-1.234, x.parse_float(-10, 10))
  end

  def test_parse_confidence
    x = PatternSearch::Term.new(:xxx)
    x.vals = [];              assert_raises(PatternSearch::MissingValueError) { x.parse_confidence }
    x.vals = [1, 2];          assert_raises(PatternSearch::TooManyValuesError) { x.parse_confidence }
    x.vals = ["xxx"];         assert_raises(PatternSearch::BadConfidenceError) { x.parse_confidence }
    x.vals = ["-100.1"];      assert_raises(PatternSearch::BadConfidenceError) { x.parse_confidence }
    x.vals = ["100.1"];       assert_raises(PatternSearch::BadConfidenceError) { x.parse_confidence }
    x.vals = ["-100"];        assert_equal([-3,3], x.parse_confidence)
    x.vals = ["100"];         assert_equal([3,3], x.parse_confidence)
    x.vals = ["90.0"];        assert_equal([90*3000, 100*3000], x.parse_confidence.map {|x| (x*100000).round})
    x.vals = ["-.123-.123"];  assert_equal([-123*3, 123*3], x.parse_confidence.map {|x| (x*100000).round})
    x.vals = ["1.234-2.345"]; assert_equal([1234*3, 2345*3], x.parse_confidence.map {|x| (x*100000).round})
  end

  def test_parse_list_of_names
    ids = Name.all[3..5].map(&:id)
    x = PatternSearch::Term.new(:xxx)
    x.vals = [];                   assert_raises(PatternSearch::MissingValueError) { x.parse_list_of_names }
    x.vals = ["Coprinus comatus"]; assert_equal([names(:coprinus_comatus).id], x.parse_list_of_names)
    x.vals = [ids.first.to_s];     assert_equal([ids.first], x.parse_list_of_names)
    x.vals = ids.map(&:to_s);      assert_equal(ids, x.parse_list_of_names)
  end

  def test_parse_list_of_locations
    ids = Location.all[3..5].map(&:id)
    x = PatternSearch::Term.new(:xxx)
    x.vals = [];                           assert_raises(PatternSearch::MissingValueError) { x.parse_list_of_locations }
    x.vals = ["USA, California, Burbank"]; assert_equal([locations(:burbank).id], x.parse_list_of_locations)
    x.vals = ["Burbank, California, USA"]; assert_equal([locations(:burbank).id], x.parse_list_of_locations)
    x.vals = [ids.first.to_s];             assert_equal([ids.first], x.parse_list_of_locations)
    x.vals = ids.map(&:to_s);              assert_equal(ids, x.parse_list_of_locations)
  end

  def test_parse_list_of_projects
    ids = Project.all[3..5].map(&:id)
    x = PatternSearch::Term.new(:xxx)
    x.vals = [];                 assert_raises(PatternSearch::MissingValueError) { x.parse_list_of_projects }
    x.vals = ["Bolete Project"]; assert_equal([projects(:bolete_project).id], x.parse_list_of_projects)
    x.vals = [ids.first.to_s];   assert_equal([ids.first], x.parse_list_of_projects)
    x.vals = ids.map(&:to_s);    assert_equal(ids, x.parse_list_of_projects)
  end

  def test_parse_list_of_species_lists
    ids = SpeciesList.all[3..5].map(&:id)
    x = PatternSearch::Term.new(:xxx)
    x.vals = [];                    assert_raises(PatternSearch::MissingValueError) { x.parse_list_of_species_lists }
    x.vals = ["List of mysteries"]; assert_equal([species_lists(:unknown_species_list).id], x.parse_list_of_species_lists)
    x.vals = [ids.first.to_s];      assert_equal([ids.first], x.parse_list_of_species_lists)
    x.vals = ids.map(&:to_s);       assert_equal(ids, x.parse_list_of_species_lists)
  end

  def test_parse_list_of_users
    x = PatternSearch::Term.new(:xxx)
    x.vals = [];              assert_raises(PatternSearch::MissingValueError) { x.parse_list_of_users }
    x.vals = [mary.id.to_s];  assert_equal([mary.id], x.parse_list_of_users)
    x.vals = ["katrina"];     assert_equal([katrina.id], x.parse_list_of_users)
    x.vals = ["Tricky Dick"]; assert_equal([dick.id], x.parse_list_of_users)
  ...
  def test_observation_search_name
    ...
    assert(expect.count > 0)
    x = PatternSearch::Observation.new('name:"Conocybe filaris","Boletus edulis Bull."')
    assert_obj_list_equal(expect.sort_by(&:id), x.query.results.sort_by(&:id))
  end
  ...
  def test_observation_search_gps
    ...
    assert(expect.count > 0)
    x = PatternSearch::Observation.new("west:-118.4 east:-118.3 north:34.2 south:34.1")
    assert_obj_list_equal(expect.sort_by(&:id), x.query.results.sort_by(&:id))
  end
  ...
  def test_observation_search_has_notes_no
    expect = Observation.where(notes: "")
    assert(expect.count > 0)
  ...
  def test_observation_search_has_notes_yes
    expect = Observation.where('notes IS NOT NULL AND notes != ""')
    assert(expect.count > 0)
