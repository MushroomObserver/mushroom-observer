...
class ObjectLinkHelperTest < ActionView::TestCase
  def test_name_link
    ...
    assert_equal(expected_link(path, link_text), name_link(name.id))

    link_text = name.display_name_brief_authors.t
    assert_equal(expected_link(path, link_text), name_link(name))
  end
  ...
  def test_link_if_object
    ...
    assert_equal(expected_link(path, link_text),
  ...
  def expected_link(path, link_text)
    "<a href=\"#{path}\">#{link_text}</a>"
  end
end
