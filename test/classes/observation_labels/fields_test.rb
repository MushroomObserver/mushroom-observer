...
class FieldsTest < UnitTestCase
  def test_error_case
    ...
    assert_includes(similar_obs, observations(:nearby_megacollybia_rodmanii_obs))
    assert_not_includes(similar_obs,
                        observations(:faraway_megacollybia_rodmanii_obs))
    assert_not_includes(similar_obs, observations(:older_megacollybia_rodmanii_obs))
    assert_not_includes(similar_obs, observations(:megacollybia_platyphylla_obs))
  end
end
