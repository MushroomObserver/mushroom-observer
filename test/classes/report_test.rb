...
class ReportTest < UnitTestCase
  ...
  # MycoportalImageList license filtering
  def test_mycoportal_image_list_excludes_restricted_license
    obs = observations(:agaricus_campestris_obs)
    query = Query.lookup(:Observation, id_in_set: [obs.id])
    report = Report::MycoportalImageList.new(query: query)
    table = CSV.parse(report.body)

    assert_equal(1, table.count,
                 "Only header row; ccnc30 image should be excluded")
  end

  def test_mycoportal_image_list_includes_public_domain
    obs = observations(:peltigera_obs)
    img = images(:peltigera_image)
    query = Query.lookup(:Observation, id_in_set: [obs.id])
    report = Report::MycoportalImageList.new(query: query)
    table = CSV.parse(report.body)

    assert_equal(2, table.count,
                 "Header + one publicdomain image row")
    assert_equal(["MUOB #{obs.id}",
                  "https://mushroomobserver.org/images/1280/#{img.id}.jpg"],
                 table[1],
                 "Row should contain catalogNumber and image URL")
  end

  def test_mycoportal_image_list_mixed_licenses
    # fungi_obs has conic_image (ccnc30, excluded) and
    # plane_image_example (publicdomain, included)
    obs = observations(:fungi_obs)
    img = images(:plane_image_example)
    query = Query.lookup(:Observation, id_in_set: [obs.id])
    report = Report::MycoportalImageList.new(query: query)
    table = CSV.parse(report.body)

    assert_equal(2, table.count,
                 "Header + one publicdomain image; ccnc30 image excluded")
    assert_equal(["MUOB #{obs.id}",
                  "https://mushroomobserver.org/images/1280/#{img.id}.jpg"],
                 table[1],
                 "Only the open-license image should appear")
  end

  def test_mycoportal_image_list_includes_ccby
    obs = observations(:minimal_unknown_obs)
    img = Image.create!(user: rolf, when: Time.zone.now,
    ...
    assert_equal(2, table.count, "Header + one ccby image row")
  end

  def test_mycoportal_image_list_includes_ccbync
    obs = observations(:minimal_unknown_obs)
    img = Image.create!(user: rolf, when: Time.zone.now,
    ...
    assert_equal(2, table.count, "Header + one ccbync image row")
  end
