...
class Query::ExternalLinksTest < UnitTestCase
  ...
  def test_external_link_order_by_external_id
    expects = ExternalLink.order_by(:external_id)
    assert_query(expects, :ExternalLink, order_by: :external_id)
  end
  ...
  def test_external_link_observations
    ...
    expects = obs.external_links.sort_by(&:external_id)
  ...
  def test_external_link_external_sites
    ...
    sites.each do |site|
      expects = site.external_links.sort_by(&:external_id)
