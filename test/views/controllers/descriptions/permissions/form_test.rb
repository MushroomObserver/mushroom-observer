...
module Views::Controllers::Descriptions::Permissions
  class FormTest < ComponentTestCase
    ...
    def test_name_description_form
      html = render_form
      ...
      # 6 writein rows with autocompleter and permission checkboxes
      (1..6).each do |i|
        assert_html(html,
    ...
    def test_with_writein_data_populated
      data = {
        1 => { name: "katrina", reader: true,
               writer: false, admin: false }
      }
      html = render_form(@description, @groups, data)

      assert_html(html,
                  "input[name='description_permissions" \
                  "[writein_name_1]'][value='katrina']")
    end

    private

    # Sibling reference within the namespace — `Form` resolves to
    # `Views::Controllers::Descriptions::Permissions::Form`.
    def render_form(description = @description,
                    groups = @groups, data = nil)
      render(Form.new(
