...
module Views::Controllers::Admin::Emails::WebmasterQuestions
  class FormTest < ComponentTestCase
    ...
    def test_renders_form_with_help_note
      # `.tp` wraps the rendered text in `<div class="textile">...</div>`;
      # scope the selector there (the outer `<p>` from `p { :foo.tp }`
      # only contains the textile div, not direct text).
      assert_html(@html, ".textile",
                  text: :ask_webmaster_note.tp.as_displayed)
    end
    ...
    def render_form(reply_to: nil, message: nil, email_error: false)
      form = Form.new(reply_to: reply_to,
                      message: message,
