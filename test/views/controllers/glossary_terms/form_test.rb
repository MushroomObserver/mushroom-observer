...
module Views::Controllers::GlossaryTerms
  class FormTest < ComponentTestCase
    ...
    def test_renders_form_with_name_field
      assert_html(@html, "label[for='glossary_term_name']",
                  text: :glossary_term_name.l)
      assert_html(@html,
                  "input[name='glossary_term[name]'][data-autofocus]")
    end

    def test_renders_form_with_description_field
      assert_html(@html, "label[for='glossary_term_description']",
                  text: :glossary_term_description.l)
      assert_html(@html,
                  "textarea[name='glossary_term[description]'][rows='16']")
    end
    ...
    def test_form_has_correct_attributes_for_existing_record
      ...
      assert_html(html,
                  "form[action='/glossary_terms/#{@glossary_term.id}']")
