...
module Views::Controllers::Info
  # NOTE: Page title and help block are rendered by the view template
  # (Views::Controllers::Info::TextileSandbox), not the form. Those
  # should be tested in controller/integration tests.
  class TextileSandboxFormTest < ComponentTestCase
    def test_initial_form
      ...
      assert_html(html, "label[for='textile_sandbox_code']",
                  text: "#{:sandbox_enter.t}:")
      ...
      # Quick reference section — strong wrapped in <p> inside .mt-3
      assert_html(html, ".mt-3 p strong",
                  text: "#{:sandbox_quick_ref.t}:")
      assert_html(html, "pre")
      assert_includes(html, "&micro;")
      assert_includes(html, "&deg;")

      # More help and web references — text-only (multiple sibling
      # `<strong>` tags share the same parent; element-level scoping
      # is awkward).
      assert_includes(html, "#{:sandbox_more_help.t}:".as_displayed)
      assert_includes(html, "#{:sandbox_web_refs.t}:".as_displayed)
      assert_html(html,
    ...
    def test_form_with_result
      ...
      assert_html(html, "strong", text: "#{:sandbox_look_like.t}:")
    ...
    def render_form(code:, show_result:, submit_type: nil)
      ...
      render(TextileSandboxForm.new(model,
