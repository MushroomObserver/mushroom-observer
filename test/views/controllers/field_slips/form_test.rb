...
# Tests for Views::Controllers::FieldSlips::Form — the Phlex
# conversion of the field-slip form (replaces
# app/views/controllers/field_slips/_form.erb plus its matrix
# sub-partials).
#
# Focus areas:
...
# 5. Observation matrix FieldProxy param wiring — the most
#    regression-prone path (flat `observation_ids[]` + namespaced
...
module Views::Controllers::FieldSlips
  class FormTest < ComponentTestCase
    ...
    def test_species_list_hidden_field_carries_passed_value
      ...
      assert_html(html,
                  "input[type='hidden'][name='species_list'][value='42']")
    end
    ...
    def test_project_select_includes_nil_sentinel_option
      ...
      # FieldSlip#projects always unshifts a `[nil_label, nil]`
      # sentinel, so the select always includes the "no project"
      # option even when a real project is selected. This guards
      # against a refactor that would drop the sentinel and break the
      # "unset project" UX.
      assert_html(
        html,
    ...
    #
    # CRITICAL: these tests pin the param-name contract between the
    # form and field_slips_controller. A change to the FieldProxy
    # namespace would silently break form submission — this is the
    # only test that would catch it. (Coverage audit: no other tests
    # anywhere assert on `observation_ids[]` or
    ...
    def test_new_action_matrix_uses_flat_observation_ids
      ...
      # because FieldSlip has no `observation_ids=` accessor (join is
      # via the occurrence). The controller reads
      # `params[:observation_ids]` directly.
      recent.each do |obs|
    ...
    def test_no_observation_matrix_on_new_form_without_recent_obs
      ...
      assert_no_html(html,
                     "input[name='field_slip[primary_observation_id]']")
    end
    ...
    def render_form(model, **)
      render(Form.new(model, user: @user, **))
    end
  end
end
