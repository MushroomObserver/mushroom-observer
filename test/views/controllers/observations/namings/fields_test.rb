...
module Views::Controllers::Observations::Namings
  ...
  class FieldsTest < ComponentTestCase
    ...
    # Test for bug: collapseFields target must use namespaced
    # controller format. The autocompleter--name controller requires
    # `data-autocompleter--name-target`, not `data-autocompleter-target`.
    def test_collapse_fields_uses_correct_stimulus_target
      html = render_naming_form(create: true)

      # Should use the namespaced target format for autocompleter--name.
      assert_html(
    ...
    def render_naming_form(create: true, show_reasons: true,
                           context: "lightbox")
      component = Form.new(
        @naming,
