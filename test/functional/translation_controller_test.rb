...
class TranslationControllerTest < FunctionalTestCase
  ...
  def assert_major_header(str, item)
    assert(item.is_a? TranslationFormMajorHeader)
    assert_equal(str, item.string)
  end

  def assert_minor_header(str, item)
    assert(item.is_a? TranslationFormMinorHeader)
    assert_equal(str, item.string)
  end

  def assert_comment(str, item)
    assert(item.is_a? TranslationFormComment)
    assert_equal(str, item.string)
  end

  def assert_tag_field(tag, item)
    assert(item.is_a? TranslationFormTagField)
...
################################################################################
  ...
  def test_tags_to_show
    lang = languages(:english)
    strings = lang.localization_strings
    assert_equal(%w(TWO TWOS four one three two twos), strings.keys.sort)
    tags1 = @controller.tags_to_show(['one'], strings)
    tags2 = @controller.tags_to_show(['two'], strings)
    tags3 = @controller.tags_to_show(['Two'], strings)
    tags4 = @controller.tags_to_show(['TWOS'], strings)
    tags5 = @controller.tags_to_show(['tWoS', 'FoUr'], strings)
