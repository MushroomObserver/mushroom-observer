# This file contains the BrowserStatus module, and a few handly related methods
...
#--
# This module is automatically included by rails.
# These methods are available in every view template.
#++
...
module ApplicationHelper
  ...
  def report_browser_status
    ...
    if @session_working && !@js && !session[:js_override] &&
      request.method == "get"
  ...
  def add_args_to_url(url, new_args)
    BrowserStatusPrivate::add_args_to_url(url, new_args)
  end
end
...
module BrowserStatus
  ...
  def browser_status
    ...
    @session_working = true if session[:_working]
    session[:_working] = true

    # Are cookies enabled?
    @cookies_enabled = true if cookies[:_enabled]
    cookies[:_enabled] = "1"
    ...
    if ua = BrowserStatusPrivate::user_agent(env)
      @ua[ua] = true
  ...
  #   link_to("Next Page", reload_with_args(:page => 2))
  def reload_with_args(new_args)
    redirect_to add_args_to_url(request.request_uri, new_args)
  end
...
class BrowserStatusPrivate  # :nodoc: all
  def BrowserStatusPrivate::add_args_to_url(url, new_args)
    args = {}
  ...
  def BrowserStatusPrivate::user_agent(ua)
    ...
    return :robot if ua.match(/PBWF|fouineur|Ask Jeeves.Teoma|Black Widow|FDSE robot|Pimptrain's robot|ChristCrawler|sharp-info-agent|SpiderView|Sleek Spider/)
