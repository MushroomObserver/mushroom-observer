...
# Form for creating or editing locations. Includes an interactive
# map, compass inputs for coordinates, and elevation inputs. Renders
# a read-only display variant for locked locations when the current
# user is not in admin mode.
#
# @example Usage
#   render(Views::Controllers::Locations::Form.new(
#     @location,
...
module Views::Controllers::Locations
  class Form < ::Components::ApplicationForm
    ...
    def render_map
      Map(objects: [model], editable: true, map_type: "location",
          controller: nil)
    end
    ...
    def form_action
      create? ? create_action_path : update_action_path
    end

    def create_action_path
      url_for(controller: "locations", action: :create,
              where: @original_name, approved_where: @display_name,
              set_observation: @set_observation,
              set_species_list: @set_species_list,
    ...
    def update_action_path
      url_for(controller: "locations", action: :update,
              id: model.id, approved_where: @display_name,
              only_path: true)
    end
  end
end
