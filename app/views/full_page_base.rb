...
class Views::FullPageBase < Views::Base
  ...
  # ----- Page-title chrome ------------------------------------------------
  #
  # Each method writes to `content_for(:title)` (visible page heading,
  # HTML-safe), `content_for(:document_title)` (browser-tab `<title>`,
  # plain text), and — for index titles — `content_for(:filters)` for
  ...
  # (or its type-tag label) prefixed with `Type id:`. `capture` is
  # needed because Phlex's `render` emits to the buffer rather than
  # returning a string — we want the HTML as a string to stash in
  # `content_for(:title)`.
