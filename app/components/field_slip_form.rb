...
# Replaces app/views/controllers/field_slips/_form.html.erb plus its
# matrix sub-partials (_recent_observations, _edit_observations).
#
# When the field slip has no code yet, renders a minimal "enter code"
# form that GETs to /field_slips/new. Otherwise renders the full
...
class Components::FieldSlipForm < Components::ApplicationForm
  ...
  def render_observation_matrix(observations, checked_ids:, primary_id:)
    # FieldSlip submits matrix params as flat `observation_ids[]` and
    # namespaced `field_slip[primary_observation_id]` — both unchanged
    # from the ERB version, so the controller doesn't need to move.
    # Since FieldSlip doesn't have its own `observation_ids=` accessor
    # (the join is via the occurrence), we use `FieldProxy` directly
    # instead of `field(:foo).checkbox`.
