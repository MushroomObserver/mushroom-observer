...
class Components::ApplicationForm < Superform::Rails::Form
  ...
  def password_field(field_name, **options)
    ...
    # Match ERB `password_field_with_label`: default the input value to ""
    # (prevents Rails from filling the field with the model's stored
    # password hash when the form re-renders after a validation error).
  ...
  def number_field(field_name, **options)
    ...
    # who want a different minimum pass `min: <n>` explicitly; `min: nil`
    # opts out.
