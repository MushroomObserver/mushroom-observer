...
# copyright, notes, projects).
# Replaces app/views/controllers/observations/images/edit.html.erb
# plus its `_fields_for_images` and `_project` partials.
#
...
class Components::ImageEditForm < Components::ApplicationForm
  # Render state is bundled into the `**state` splat so the init
  # stays under Metrics/ParameterLists. Callers still pass each piece
  # as a named kwarg (projects:, submitted_project_ids:).
  ...
  # Same permission rule as the ERB partial:
