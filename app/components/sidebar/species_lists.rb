...
module Components
  module Sidebar
    ...
    #
    # @example Basic usage
    #   render(Components::Sidebar::SpeciesLists.new(user: current_user, classes: sidebar_css_classes))
    #
    class SpeciesLists < Components::Base
      ...
      def view_template
        div(class: @classes[:heading]) do
          plain("#{:app_species_list.t}:")
        end

        sidebar_species_lists_tabs(@user).compact.each do |link|
