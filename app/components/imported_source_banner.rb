...
#
# Hides silently if the obs has no external_source. If the source
# is set but no per-observation URL is known (e.g. a source whose
# `observation_url` returns nil), the credit is rendered as plain
# text without a link; the (?) widget still appears.
class Components::ImportedSourceBanner < Components::Base
  HELP_ARTICLE_ID = 39
