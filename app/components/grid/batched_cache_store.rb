...
# Cache-store wrapper for one Grid#render_cached_boxes call.
# Phlex's `low_level_cache` calls `cache_store.fetch(key) { block }`
# once per object -- against the underlying store, that's one round
# trip per object (Solid Cache is DB-backed, so N objects means N SQL
...
# unmodified, it just talks to this instead of the underlying store.
class Components::Grid::BatchedCacheStore
  ...
  # them. `render_cached_boxes` has one `low_level_cache` call
  # site today, so there's only one options value in play; a
  # `fetch(key, **options)` call still accepts (but doesn't use)
  # `options` to match Phlex's cache_store interface.
