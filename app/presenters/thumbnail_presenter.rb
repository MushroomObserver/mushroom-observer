...
class ThumbnailPresenter
  attr_accessor \
    ...
    :img_link,      # stretched-link (link/button/form)
  ...
  def args_to_presenter(image, image_id, args)
    ...
    self.img_tag = image_tag(img_src, html_options)
    self.img_link = image_link_html(args[:link], args[:link_method])
    self.lightbox_link = lightbox_link(lb_url, lb_id, lb_caption)
    self.votes = args[:votes]
    self.img_filename = img_filename(args, image)
  end
