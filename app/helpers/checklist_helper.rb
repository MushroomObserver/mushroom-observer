...
module ChecklistHelper
  def checklist_name_link(taxon:, user:, project:, location:, list:, data:)
    name, name_id, deprecated, synonym_id = taxon
    link = checklist_name_link_path(name_id, user, project, location, list)
    content = tag.i(name)
  ...
  def checklist_name_link_path(name_id, user, project, location, list)
    prefix = if user
               "user:#{user.id}"
             elsif project
               "project:#{project.id}"
