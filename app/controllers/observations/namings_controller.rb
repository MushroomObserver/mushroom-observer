...
# NOTE: This controller formerly initiated many superfluous N+1 queries.
# It's being carefully untangled now.
# The strategy is the observation_id is always passed to naming and vote routes.
# From the observation_id, we can eager load the naming_includes on the obs:
# Not only votes on this naming, but all the namings, names, votes and users.
# Passed to the consensus calculator, it can figure out the new consensus name
# without reloading anything until it's done, and the reload is relevant.
# Please be careful altering the way any of the associations are loaded.
module Observations
