...
module Validations
  ...
  def check_date(when_date, errors)
    ...
    errors.add(:when, :validate_when_future,
               when: when_date, today: Time.zone.today)
  ...
  def check_time(when_date, errors)
    ...
    errors.add(:when, :validate_when_future_debug,
               when: when_date, detail: "Time.now=#{6.hours.from_now}")
    errors.add(:when, :validate_future_time)
    false
  end
  ...
  def check_year(when_date, errors)
    ...
    errors.add(:when, :validate_when_selected, when: when_date)
