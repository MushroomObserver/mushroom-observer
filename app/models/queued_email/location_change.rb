...
class QueuedEmail::LocationChange < QueuedEmail
  ...
  def description
    get_object(:description, ::Location::Description)
  end
  ...
  def self.create_email(sender, recipient, location, desc = nil)
    ...
    if location
      ...
      result.add_integer(
        ...
        (location.saved_changes? ? location.version - 1 : location.version)
      )
    ...
    if desc
      ...
      result.add_integer(:old_description_version, (
                         desc.saved_changes? ? desc.version - 1 : desc.version))
