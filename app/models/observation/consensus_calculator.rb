    ...
    def calc(debug)
      ...
      @namings.each do |naming|
        ...
        naming.votes.each do |vote|
          ...
          if debug
            @debug_messages += "raw vote: taxon_id=#{taxon_id}, name_id=#{name_id}, " \
      ...
      taxon_votes.each_key do |taxon_id|
        ...
        taxon_votes[taxon_id].each_key do |user_id|
          ...
          @debug_messages += "vote: taxon_id=#{taxon_id}, user_id=#{user_id}, " \
                             "val=#{val}, wgt=#{wgt}<br/>"
        end
      end
      ...
      votes.each_key do |taxon_id|
        ...
        if debug
          @debug_messages += "#{taxon_id}: val=#{val} wgt=#{wgt} age=#{age}<br/>"
        end
      ...
      if debug
        @debug_messages += "best: id=#{best_id}, val=#{best_val}, wgt=#{best_wgt}, " \
                           "age=#{best_age}<br/>"
      end
      ...
      if debug
        @debug_messages += "unmash: best=#{best ? best.real_text_name : "nil"}<br/>"
      end
      ...
      if best&.synonym_id
        ...
        if names.length == 1
          ...
          if debug
            @debug_messages += "Multiple synonyms: #{names.map(&:id).join(", ")}<br>"
          end
          ...
          name_votes.each_key do |name_id|
            ...
            name_votes[name_id].each_key do |user_id|
              ...
              @debug_messages += "vote: name_id=#{name_id}, user_id=#{user_id}, " \
                                 "val=#{val}, wgt=#{wgt}<br/>"
            end
          end
          ...
          names.each do |name|
            ...
            if debug
              @debug_messages += "#{name_id}: val=#{val} wgt=#{wgt} " \
                                 "age=#{age}<br/>"
            end
          ...
          if debug
            @debug_messages += "best: id=#{best_id2}, val=#{best_val2}, " \
      ...
      best ||= Name.unknown
      if debug
        @debug_messages += "fallback: best=#{best ? best.real_text_name : "nil"}"
      end
