...
class QueuedEmail < ActiveRecord::Base
  ...
  # Dump out all the info about a QueuedEmail to a string.
  def dump
    result = ''
    result += "#{self.id}: from => #{self.user and self.user.login}, to => #{self.to_user.login}, flavor => #{self.flavor}, queued => #{self.queued}\n"
