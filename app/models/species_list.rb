...
class SpeciesList < AbstractModel
  ...
  def file=(file_field)
    if file_field.respond_to?(:read) &&
      ...
      when "text/plain",
        ...
        fail "Unrecognized content_type: #{content_type.inspect}"
      end
    else
      fail "Unrecognized file_field class: #{file_field.inspect}"
    end
  end
  ...
  def process_name_list(sorter)
    ...
    entry_text.each do |e|
      ...
      e.split(/\s*\r\s*/).each do |key_value|
        ...
        if kv.length != 2
          fail sprintf("Bad key|value pair (%s) in %s", key_value, filename)
        end
        key, value = kv
        if key == "Date"
          ...
          fail sprintf("Unrecognized key|value pair: %s\n", key_value)
        end
      end
  ...
  def construct_observation(name, args = {})
    fail "missing or invalid name: #{name.inspect}" unless name.is_a?(Name)
