...
class SpeciesList < AbstractModel # rubocop:disable Metrics/ClassLength
  ...
  def process_name_list(user, sorter)
    ...
    entry_text.each do |e|
      ...
      e.split(/\s*\r\s*/).each do |key_value|
        ...
        if kv.length != 2
          raise(format("Bad key|value pair (%s) in this list", key_value))
        end
      ...
      sorter.add_name(user, what, timestamp) if what
    end
  end
