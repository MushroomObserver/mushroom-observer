...
module LanguageExporter
  ...
  def localization_file
    "#{::Rails.root.to_s}/config/locales/#{lang}.yml"
  end

  # This is the hand-editable export file.
  def export_file
    "#{::Rails.root.to_s}/config/locales/#{lang}.txt"
  end
  ...
  def write_localization_file(data)
    if TESTING
      @@localization_files[lang] = data if @@localization_files
    else
  ...
  def read_export_file
    if TESTING and @@export_files
      YAML::load(@@export_files[lang].join)
    else
  ...
  def write_export_file_lines(output_lines)
    if TESTING
      @@export_files[lang] = output_lines if @@export_files
    else
