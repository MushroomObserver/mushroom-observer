...
#  the API class method +execute+.  This takes care of selecting the
...
#
#    api = API.execute(method: post, action: comment, target: etc...)
#
...
#                     Values are API::ParameterDeclaration instances.
#
...
#
#  The main four entry points (called by API.execute) are:
#
...
#
#    before_create    Called in POST after validation, before creating record.
...
#  particular API::Parser subclass, e.g., :string goes to API::Parser::String.
#
#  Creating new parser subclasses is extremely easy.  Most of the work is
#  done for you in API::Parsers::Base.  All you need to do is provide a +parse+
...
#  ActiveRecord or elsewhere in the Rails app) are subclasses of API::Error
