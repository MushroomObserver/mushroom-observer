...
class API
  class NameAPI < ModelAPI
    ...
    def build_object
      ...
      fail MissingParameter.new(:name_str) if name_str.blank?
      fail MissingParameter.new(:rank) if rank.blank?
      ...
      fail NameAlreadyExists.new(name_str, match) if match

      # Make sure the name parses.
      names = Name.find_or_create_name_and_parents(name_str2)
      name  = names.last
      fail NameDoesntParse.new(name_str2) if name.nil?
