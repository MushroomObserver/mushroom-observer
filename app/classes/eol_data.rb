...
class EolData
  ...
  IMAGE_CONDITIONS = %(FROM observations, image_observations, images, names
    WHERE observations.name_id = names.id
    AND observations.vote_cache >= 2.4
    AND observations.id = image_observations.observation_id
    AND image_observations.image_id = images.id
  ...
  GLOSSARY_TERM_CONDITIONS = %(
    FROM images, image_observations, observations, names, glossary_terms
    ...
    AND image_observations.image_id = images.id
    AND image_observations.observation_id = observations.id
