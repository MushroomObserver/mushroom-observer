...
module Query::Scopes::Descriptions
  def initialize_description_public_parameter(type)
    add_boolean_condition(
      ...
      desc_model(type)[:public].eq(true),
      desc_model(type)[:public].eq(false),
      params[:public]
    )
  end
  ...
  def add_with_desc_ids_condition(type = model.type_tag)
    ...
    @scopes = @scopes.where(desc_model(type)[:id].in(set)).
              order(
  ...
  def initialize_desc_type_parameter(type)
    add_indexed_enum_condition(
      ...
      desc_model(type)[:source_type],
      params[:desc_type],
      desc_model(type).source_types # Hash
    )
  end
  ...
  def desc_model(type)
    "#{type}_description".classify.constantize
  end

  def desc_author_model(type)
    "#{type}_description_author".classify.constantize
  end

  def desc_editor_model(type)
    "#{type}_description_editor".classify.constantize
  end
end
