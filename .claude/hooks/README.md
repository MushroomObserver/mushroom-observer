...
| `block_noop_cd.sh` | `PreToolUse` on every `Bash` call | Blocks a command whose leading `cd` resolves to the directory the Bash tool is already in — a pure no-op that also trips a separate "path resolution bypass" approval prompt when chained with `&&`/`;`. Resolves the target via a real subshell `cd` (so `~`, `$HOME`, `..`, etc. expand correctly) and only blocks on an exact match against the current directory; a `cd` to any other directory is always allowed. |
