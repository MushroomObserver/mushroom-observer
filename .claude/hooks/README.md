...
| `post_rebase_lang_db.sh` | `PostToolUse` on `Bash` containing `git rebase` / `git pull --rebase` | After a successful rebase (no mid-rebase conflict state), diffs ORIG_HEAD against HEAD. If `config/locales/en.txt` moved, runs `bin/rails lang:update` so `en.yml` reflects upstream translations. If any `db/migrate/*.rb` moved, runs `bin/rails db:migrate` (development) + `RAILS_ENV=test`. Non-blocking; surfaces failures with log paths so the human can rerun manually. |
| `check_lang_update_before_test.sh` | `PreToolUse` on `Bash` containing `rails test` | Guards a gap the checkout/rebase hooks above don't cover: `en.txt` can end up newer than the compiled `en.yml` without a checkout or rebase ever running (e.g. a `git merge`, or a locally-edited `en.txt`). Compares `en.txt`'s current git blob hash against the hash cached the last time this hook successfully ran `bin/rails lang:update`; only re-runs `lang:update` (which exports every locale, not just `en.txt`) when the hash actually changed. Non-blocking — auto-fixes and prints a one-line summary, so a stale locale cache never surfaces as a spurious `Symbol.missing_tags` test failure. |
